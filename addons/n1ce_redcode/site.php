...
class N1ce_redcodeModuleSite extends WeModuleSite {
	...
	public function doMobileSalt(){
		...
		if($settings['xianzhi'] == '2'){
			$isget = pdo_fetch('select * from ' . tablename('n1ce_red_user') . ' where uniacid = :uniacid and openid = :openid', array(':uniacid' => $_W['uniacid'],':openid' => $openid));
			if($isget){
				message('每人只限领取一次哦！','','warning');
			}
		}
		$codeinfo = pdo_fetch('select * from' . tablename('n1ce_red_code') . 'where uniacid = :uniacid and pici = :pici and salt = :salt and status = 1',array(':uniacid'=>$_W['uniacid'],':pici'=>$pici,':salt'=>$saltcode));
		if(empty($codeinfo['code']) || empty($saltcode)){
			message('二维码已经失效!','','error');
		}
		...
		if($_W['fans']['follow'] == 0) {
			...
			if($sends['type'] == '1'){
				...
				if($action === true){
					...
					message('红包发送成功,请返回微信聊天主页面查看红包消息！','','success');
				}else{
			...
			if($sends['type'] == '2'){
				...
				if($res){
					...
					$tempstr = htmlspecialchars_decode(str_replace('&quot;','&#039;',$notice),ENT_QUOTES);
					$this->sendText($openid,$tempstr);
					message('恭喜你获得一张卡券,请返回微信聊天主页面查看卡券消息！','',success);
				}
			}
			if($sends['type'] == '3'){
				pdo_update('n1ce_red_prize',$updatas,array('id'=>$pid));
			...
			if($sends['type'] == '4'){
				...
				if($res){
					...
					$this->sendText($openid,$notice);
					message('恭喜你获得积分奖励,奖励已发放,请前往会员中心查看！','','success');
				}
			}
			if($sends['type'] == '5'){
				pdo_update('n1ce_red_prize',$updatas,array('id'=>$pid));
				...
				$tempstr = htmlspecialchars_decode(str_replace('&quot;','&#039;',$notice),ENT_QUOTES);
				$this->sendText($openid,$tempstr);
				message('请返回微信聊天主页面查看公众号消息！','','success');
			}
			if($sends['type'] == '6'){
				$money = rand($sends['min_money'], $sends['max_money']);
				$insert['money'] = $money;
					if($money){
						...
						if($action === true){
							...
							message('奖品发放成功,请返回微信聊天主页面查看公众号消息！','','success');
						}else{
							$insert['status'] = '2';
							pdo_insert('n1ce_red_user', $insert);
							$actions = "亲爱的管理员，有粉丝红包领取失败！\n原因：".$action;
	...
	public function doMobileRedurl(){
		...
		$settings = $this->module['config'];
		$brrow = mc_oauth_userinfo();
		$token = "1".md5(microtime()+rand(1,10000));
	...
	public function doMobileSaltcheck(){
		...
		if($_SESSION['token'] == $token){
			...
			exit(json_encode(array('status' => 3, 'msg' => "请勿重复点击")));
		}
		$res = pdo_fetch('select * from' . tablename('n1ce_red_user') . ' where uniacid = :uniacid and salt = :salt and status = 3', array(':uniacid' => $_W['uniacid'], ':salt' => $salt));
		...
		if(!$res){
			...
			if(empty($total_num)){
				...
				$action = $this->sendRedgroupPacket($bropenid,$res['money'],$total_num);
			}
			...
			if($action === true){
				...
				pdo_update('n1ce_red_user',$binsert, array('uniacid' => $_W['uniacid'], 'salt' => $salt));
 ...
 */
	...
	public function doWebQrcode() {
		...
		foreach($res as $key => $vol){
			$keyword = $vol['code'];
			$qrcid = pdo_fetch("SELECT max(qrcid) FROM ".tablename('qrcode')." WHERE acid = :acid AND model = 1", array(':acid' => $acid));
	...
	public function doWebQrcode2() {
		...
		foreach($res as $key => $vol){
			$keyword = $vol['code'];
			$qrcid = pdo_fetch("SELECT max(qrcid) FROM ".tablename('qrcode')." WHERE model = 1 or model = 2", array(':acid' => $acid));
