<?php
		...
		if(!pdo_fieldexists('n1ce_red_code', 'salt')) {
			pdo_query("ALTER TABLE ".tablename('n1ce_red_code')." ADD `salt` varchar(32) DEFAULT '';");
		}
		...
		if(!pdo_fieldexists('n1ce_red_prize', 'prizenum')) {
			pdo_query("ALTER TABLE ".tablename('n1ce_red_prize')." ADD `prizenum` tinyint(4) NOT NULL DEFAULT '0';");
		}
		if (checksubmit()) {
			$count = $_GPC['num'];
