...
class WxpayAPI
{
	...
	{
		$data = array('nonce_str' => $params['nonce_str'], 'mch_billno' => $params['mch_billno'], 'mch_id' => $params['mch_id'], 'wxappid' => $params['wxappid'], 'send_name' => $params['send_name'], 're_openid' => $params['re_openid'], 'total_amount' => $params['total_amount'] * 100, 'total_num' => $params['total_num'], 'wishing' => $params['wishing'], 'client_ip' => $params['client_ip'], 'act_name' => $params['act_name'], 'remark' => $params['remark']);
		$xml_data = '<xml>';
		foreach ($data as $k => $v) {
			$xml_data .= "<{$k}>{$v}</{$k}>";
		}
