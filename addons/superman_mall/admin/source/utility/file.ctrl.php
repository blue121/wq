...
if ($do == 'fetch' || $do == 'upload') {
    ...
    if (defined('IN_SUPERMAN_MALL_ADMIN') && isset($_W['__shopid']) && $_W['__shopid']) {
        $new_id = pdo_insertid();
        if ($new_id) {
            $data = array(
                'shopid' => intval($_W['__shopid']),
                'attachid' => $new_id,
            );
            pdo_insert('superman_mall_shop_attachment', $data);
            if (!pdo_insertid()) {
                WeUtility::logging('fatal', '[file_upload] insert `superman_mall_shop_attachment` failed, data='.var_export($data, true));
            }
        }
    }
...
if ($do == 'delete') {
	...
	if((empty($_W['isfounder']) && $_W['role'] != 'manager') && (empty($_W['superman_mall']['shop_user']) && $_W['superman_mall']['shop_user']['groupid']!=0)) {
		exit('您没有权限删除该文件');
	}
	load()->func('file');
	if (!empty($_W['setting']['remote']['type'])) {
    ...
    if (defined('IN_SUPERMAN_MALL_ADMIN') && isset($_W['__shopid']) && $_W['__shopid']) {
        pdo_delete('superman_mall_shop_attachment', array('shopid' => intval($_W['__shopid']), 'attachid' => $id));
    }
	exit('success');
}
...
if ($do == 'local') {
    ...
    if (defined('IN_SUPERMAN_MALL_ADMIN') && isset($_W['__shopid']) && $_W['__shopid']) {
        $condition = ' WHERE a.uniacid = :uniacid AND a.type = :type AND a.id=b.attachid AND b.shopid=:shopid';
        $params = array(
            ':uniacid' => $_W['uniacid'],
            ':type' => $typeindex[$type],
            ':shopid' => intval($_W['__shopid']),
        ...
        $total = pdo_fetchcolumn('SELECT count(*) FROM '.tablename('core_attachment') ." AS a,".tablename('superman_mall_shop_attachment')." AS b {$condition}", $params);
        $sql = 'SELECT a.* FROM '.tablename('core_attachment')." AS a,";
        $sql .= tablename('superman_mall_shop_attachment')." AS b {$condition} ORDER BY a.id DESC LIMIT ".(($page-1)*$size).','.$size;
    } else {
