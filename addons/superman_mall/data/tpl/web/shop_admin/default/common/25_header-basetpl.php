...
<head>
	...
	<title><?php  if(defined('IN_SUPERMAN_MALL_ADMIN')&&$this->shop) { ?><?php  echo $this->shop['title']?><?php  } else { ?><?php  echo $this->module['config']['seo']['title']?><?php  } ?> <?php  echo $nav['title'];?><?php  if(!empty($nav['subtitle'])) { ?> - <?php  echo $nav['subtitle'];?><?php  } ?></title>
	...
	<script type="text/javascript" src="<?php  echo $_W['siteroot'];?>addons/superman_mall/template/web/shop_admin/default/js/global.js" charset="utf-8"></script>
	<?php  if(defined('IN_SUPERMAN_MALL_ADMIN')) { ?>
	<?php  $_urls = parse_url($_W['siteroot'])?>
	<script>
		var _path = '../../../../..<?php  echo $_urls["path"];?>web/resource/js/app';
		require.config({
			baseUrl: _path
		});
	</script>
